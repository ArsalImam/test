...
public class CustomHeatmapTileProvider implements TileProvider {
    ...
    private CustomHeatmapTileProvider(CustomHeatmapTileProvider.Builder builder) {
    ...
    public Tile getTile(int x, int y, int zoom) {
        ...
        if (!tileBounds.intersects(paddedBounds)) {
            ...
            if (points.isEmpty()) {
                ...
                return convertBitmap(writeTextOnDrawable(bitmap1, "x2.1"));
            }
        }
    }
    ...
    private Bitmap writeTextOnDrawable(Bitmap bm, String text) {
        ...
        paint.setTextSize(convertToPixels(DriverApp.getContext(), 18));
...
//If the text is bigger than the canvas , reduce the font size
        if (textRect.width() >= (canvas.getWidth() - 4)) //the padding on either sides is considered as 4, so as to appropriately fit in the text
            paint.setTextSize(convertToPixels(DriverApp.getContext(), 7)); //Scaling needs to be used for different dpi's
...
//"- ((paint.descent() + paint.ascent()) / 2)" is the distance from the baseline to the center.
    ...
    public static class Builder {
        ...
        public Builder() {
            this.gradient = CustomHeatmapTileProvider.DEFAULT_GRADIENT;
            this.opacity = 0.7D;
        }

        public CustomHeatmapTileProvider.Builder data(Collection<LatLng> val) {
            return this.weightedData(CustomHeatmapTileProvider.wrapData(val));
        }

        public CustomHeatmapTileProvider.Builder weightedData(Collection<WeightedLatLng> val) {
            this.data = val;
        ...
        public CustomHeatmapTileProvider.Builder radius(int val) {
            this.radius = val;
        ...
        public CustomHeatmapTileProvider.Builder gradient(Gradient val) {
            this.gradient = val;
            return this;
        }

        public CustomHeatmapTileProvider.Builder opacity(double val) {
            this.opacity = val;
        ...
        public CustomHeatmapTileProvider build() {
            if (this.data == null) {
                throw new IllegalStateException("No input data: you must use either .data or .weightedData before building");
            } else {
                return new CustomHeatmapTileProvider(this);
            }
        }
    }
}
